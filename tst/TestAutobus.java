...
class TestAutobus {
  ...
  public static void main (String[] args) {
    ...
    System.out.print('.'); nbTest++;
    new TestAutobus().testExceptionAssisNegatif();
    
    System.out.print('.'); nbTest++;
    new TestAutobus().testExceptionDeboutNegatif();

    System.out.print('.'); nbTest++;
    new TestAutobus().testExceptionCasLimite3();

    System.out.print('.'); nbTest++;
    new TestAutobus().testExceptionCasLimite4();
  ...
  protected void testCasLimite1() {
    Autobus autobus1 = null;

    autobus1 = new Autobus(-1, 1);
  }

  protected void testCasLimite2() {
    Autobus autobus2 = null;

    autobus2 = new Autobus(1, -1);
  }

  public void testExceptionAssisNegatif(){
    try{
        new TestAutobus().testCasLimite1();
  ...
  public void testExceptionDeboutNegatif(){
    try{
        new TestAutobus().testCasLimite2();
        assert false: "Exception non levée";
    }
    catch(IllegalArgumentException e){
        System.out.println("Exception levée");
  ...
  public void testInteractionDemander() {
    ...
    for (int i = 1; i < 6; i++) { //5
      bus.monteeDemanderDebout(faux[i]);
      assert 1 == faux[i].logs.size() : "erreur nombre d'appels pour " + i;
      assert "changerEnDebout" == getLastLog(faux[i]) : "mauvais appel pour " + i;
    }
  ...
  public void testInteractionChanger() {
    ...
    System.out.println(faux.logs.size());
  ...
  protected void testCasLimite3() {
    Autobus bus = new Autobus(99, 66);

    PassagerAbstrait passager = null;

    passager = new MonteeFatigue("yo", 5, ArretCalme.recuperer());

    bus.monteeDemanderAssis(passager);
    bus.monteeDemanderAssis(passager);
  }

  public void testExceptionCasLimite3(){
      try{
          new TestAutobus().testCasLimite3();
          assert false: "Exception non levée";
      }
      catch(IllegalStateException e){
          System.out.println("Exception levée");
  ...
  protected void testCasLimite4() {
    ...
    bus.monteeDemanderDebout(passager);
    bus.monteeDemanderDebout(passager);
  }

  public void testExceptionCasLimite4(){
      try{
          new TestAutobus().testCasLimite4();
          assert false: "Exception non levée";
      }
      catch(IllegalStateException e){
          System.out.println("Exception levée");
